...
public branch_changer_aux<typename std::common_type<Funcs...>::type> 
{
	...
	#define _BRANCHES pack_size<Funcs...>

	static_assert(_BRANCHES > 1);

	private:
		uint64_t current_direction;
		unsigned char jump_offsets[_BRANCHES][_DWORD];
	...
	public:
		BranchChanger(const Funcs... funcs) : current_direction(-1) 
		{
			...
			*this->bytecode_to_edit++ = _JUMP_OPCODE;
			if (_BRANCHES == 2) 
			{
		...
		void set_direction(const uint64_t condition) 
		{
			...
			{
				std::memcpy(this->bytecode_to_edit, jump_offsets[condition], _DWORD);
